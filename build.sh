...
if [[ -z $(ls -A | grep -E 'clang-neutron|clang-weebx|clang-greenforce|clang-trb|clang-playground') ]]; then
    ...
    read -p "$(tput setaf 171)👉 Which version of Clang would you like to use? (1) clang-neutron, (2) clang-weebx, (3) clang-greenforce, (4) clang-trb or (5) clang-playground:$(tput sgr0) " VERSION_CHOICE
    ...
    # Show which clang is being used
    echo -e "${greenish_yellow} Currently using Clang version: $CLANG_VERSION ${white}"
